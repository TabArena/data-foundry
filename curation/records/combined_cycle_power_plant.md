...
This is more IID scientific discvoery than a non-forecasting predictive task. But without time, this forecasting is also not possible
...
points by the sensors" -> in other words, you would have the target whenever you have the features. So it is clearly a wrong setup and we cannot recover it! But anyhow, maybe a nice time-independent task, or a causal task? Or one forecasts teh other variables and then uses the model, thus making it IID?
