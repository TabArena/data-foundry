...
Features in OpenML version do not conceptualize a task. All of them would be known when the target is known, or leak the target even. Otherwise, it might be a forecasting task too. Also, target might be multiple in some way or form.
...
Very unclear how to build a real predictive task from this and with what goal and if it would be ethical (and not just a biased model)
