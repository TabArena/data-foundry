---
unique_name: customer_personality_analysis
name: Customer_Personality_Analysis
tags:
- 2nd Tier / Scientfic Discovery
collections:
- TabArena Reject
year: '2021'
source_links:
- https://www.kaggle.com/datasets/imakash3011/customer-personality-analysis/data
source_row: 812
needs_review:
- suggestion
type_adapter_id: curation-record-v1
---

# Customer_Personality_Analysis

## Comments

CC: "Looks like a nice dataset, but source is not entirely clear to me. Also might be more a clusterting task than predictive task? - could make it a predictive task using the following feature as target: Response: 1 if customer accepted the offer in the last campaign, 0 otherwise. However, we don't know enough about the time when the features were collected, so ratehr ot use this task"

## Reference

Kaggle
