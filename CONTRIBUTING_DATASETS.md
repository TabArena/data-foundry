...
## 2. The curation pipeline, step by step

Every notebook follows the same steps, mirrored by
...
print(curated.describe())
```

Nothing is written yet — check the bundle first.

### 2.6 Run the bundle checks

```python
from data_foundry.bundle_checks import run_bundle_checks

report = run_bundle_checks(curated, ignore=[])
report.raise_if_errors()
```

These are the checks that need the *whole* bundle, so they cannot run any
earlier: whether the columns named in the task metadata exist and have a usable
dtype, whether the split indices are positional and in bounds, whether a
temporal split really only trains on the past, whether every fold's train set
covers the classes its test set contains, whether the `data_tags` agree with the
split regime the task declares, and whether the BibTeX will compile.

Each finding carries a stable slug. **Errors** must be fixed. **Warnings** are a
judgment call — when one is correct for your dataset (e.g. this dataset really
does contain duplicate rows), accept it with `ignore=["<slug>"]` and say why in a
comment next to it. That comment is the audit trail a reviewer reads.

### 2.7 Save and verify the export
...
`verify_saved_container` reloads what landed on disk and compares the file
inventory, the recomputed checksum, every column dtype, the values, and the three
metadata objects against the in-memory bundle — the one check that can only run
*after* export.
...
2. **Run `/verify-dataset <notebook>`** (slash command in
   ...
   at first.
3. **Keep raw downloads out of the repo.** The `local-data-warehouse/` tree
   is gitignored — that's intentional. Only the notebook lands in the PR.
4. **Reference the parsed metadata source** in the PR description (e.g. a
   row of the curation spreadsheet, an issue, a Slack thread) so reviewers
   can audit the field mappings.
5. **Mark TODOs explicitly.** If you can't resolve something (e.g. the BibTeX
