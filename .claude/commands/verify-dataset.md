...
| 6 | **Circular / definitional features** | Is any feature the actual criterion used to assign the label, rather than an independent measurement — e.g. a diagnostic threshold that *is* how the class was determined? Check for a single feature that near-perfectly separates one class (a quick AUC/threshold check on the suspicious column is enough evidence) or a target region for regression. Distinct from leakage (item 5): the feature is available at prediction time, but predicting it is circular, not predictive. Also check whether every feature and the label were read from the same source at the same time with no gap in which a practitioner's action could differ from just reading the label directly (e.g. `biomechanical_orthopaedic_prediction`: `degree_spondylolisthesis` is the diagnostic criterion for the Spondylolisthesis class, not a predictor of it). |
| 7 | **Irreversible leakage** | Any feature that is itself the output of a supervised transform fit on the whole dataset (discriminant score, target/mean encoding, a model's prediction, PCA of the full set)? That cannot be recomputed per split and is an exclusion, not a warning. |
| 8 | **Comments vs code** | Is every claim in `curation_comments` actually implemented in the notebook, and is every non-obvious code step documented? Silent drops, filters, and casts are the ones that bite. |
| 9 | **dtype semantics** | Are `category` / `string` / numeric / datetime chosen by *meaning* (finite value set vs. free text), proxy missing values converted to `NA`, uninformative identifiers dropped, informative ones kept and processed? Use the `dataset_missing_value_*` / `dataset_identifier_column` warnings as leads. |
| 10 | **Target & metric** | Is the target the original task's target, and is `objective_metric_name` the metric the original task/competition scored? If the checks flagged `task_metric_unknown`, confirm the custom metric is intended and registered downstream. |
| 11 | **License & citation** | Is the license what the source actually states (the checks only see whether the field is filled)? Does the BibTeX cite the *right* work — the paper/competition that published this data, not a paper that merely used it? Syntax being valid says nothing about correctness. Trace *upstream* of a GitHub/code-repo mirror to whatever paper's supplementary data it repackages — the repo's own license (or lack of one) does not govern data the repo merely redistributes (e.g. `biogeographical_ancestry_prediction`: the paper repo has no license file, but part of the data traces to a CC BY-NC-ND-licensed journal supplement). |
| 12 | **Reproducibility** | Would `download_description`, pasted into a shell today, recreate the raw inputs? Are URLs pinned (DOI, archived release) rather than mutable HEAD links? Where the notebook's `original_dataset_source_download_link` and its `download_description` disagree on which repo/file to use, that split itself is evidence something drifted — verify both resolve to the same source. |
| 13 | **Ethics & representativeness** | Any subject/creator objection to ML use, obvious ethical concern, or a task tabular models would not be used for (e.g. features that are an algorithmic vectorization of image content)? See the exclusion criteria in `/triage-candidates`. |
| 14 | **Trivial** | Do the committed check outputs suggest every model would score identically or solve it perfectly? If so, flag it — a trivial task is an exclusion. |
| 15 | **Record pointer** | Does the record's `notebook_path` name *this* notebook, is it under the tree the dataset ships from (`datasets/beyond_iid/` for BeyondArena — never `datasets/_dev/`, which holds work in progress and superseded copies), and does this notebook's saved output carry the UUID the collection pins (`BEYOND_ARENA` entry / `datasets/beyond_iid/final_uuid_list.py`)? A mismatch means the record points at the wrong run, or the notebook was re-run after the collection was pinned — say which. `data-foundry-curation sync-notebooks --check` must be clean; evidence is the UUID string itself. |

Load the selection criteria and processing conventions from `/triage-candidates` before judging items 1–4 and
13–14; they encode decisions you would otherwise guess at.
